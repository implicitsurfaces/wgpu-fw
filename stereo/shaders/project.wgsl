...
@group(4) @binding(0) var<storage,read>       feature_idx_a_buffer: array<u32>;
@group(5) @binding(0) var<storage,read>       feature_idx_b_buffer: array<u32>;


@compute @workgroup_size(64)
fn main(@builtin(global_invocation_id) global_id: vec3u) {
    let index_number: u32 = global_id.x + feature_range.feature_start;
    if index_number >= arrayLength(&feature_idx_a_buffer) ||
       index_number >= arrayLength(&feature_idx_b_buffer) ||
       index_number > feature_range.feature_end
    {
        return;
    }
    let src_feature_a_idx: u32 = feature_idx_a_buffer[index_number];
    let src_feature_b_idx: u32 = feature_idx_b_buffer[index_number];
    let dst_feature_idx:   u32 = global_id.x;
    if src_feature_a_idx >= arrayLength(&src_scene_features_a) ||
       src_feature_b_idx >= arrayLength(&src_scene_features_b) {
        return;
    }
    if dst_feature_idx >= arrayLength(&dst_img_features)   { return; }
    
    let src_scene_feature_a: SceneFeature = src_scene_features_a[src_feature_a_idx];
    let src_scene_feature_b: SceneFeature = src_scene_features_b[src_feature_b_idx];
